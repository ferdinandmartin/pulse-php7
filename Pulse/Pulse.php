...
class Pulse
{
    ...
    protected $hooks = array(
        'pulse.before' => array(array()),
        'pulse.before.router' => array(array()),
        'pulse.before.dispatch' => array(array()),
        'pulse.after.dispatch' => array(array()),
        'pulse.after.router' => array(array()),
        'pulse.after' => array(array())
    );
    ...
    *******************************************************************************/
    ...
    {
        // Setup IoC container
        $this->container = new \Pulse\Security\Set();
        $this->container['settings'] = array_merge(static::getDefaultSettings(), $userSettings);

        // Default environment
        $this->container->singleton('environment', function ($c) {
            return \Pulse\Environment::getInstance();
        });

        // Default request
        $this->container->singleton('request', function ($c) {
            return new \Pulse\Http\Request($c['environment']);
        });

        // Default response
        $this->container->singleton('response', function ($c) {
            return new \Pulse\Http\Response();
        });

        // Default router
        $this->container->singleton('router', function ($c) {
            return new \Pulse\Router();
        });
        ...
        $this->container->singleton('view', function ($c) {
            ...
            $view = ($viewClass instanceOf \Pulse\View) ? $viewClass : new $viewClass;
        ...
        $this->container->singleton('logWriter', function ($c) {
            ...
            return is_object($logWriter) ? $logWriter : new \Pulse\LogWriter($c['environment']['pulse.errors']);
        });

        // Default log
        $this->container->singleton('log', function ($c) {
            $log = new \Pulse\Log($c['logWriter']);
        ...
        $this->add(new \Pulse\Kernel\Flash());
        $this->add(new \Pulse\Kernel\MethodOverride());
        $this->add(new \Pulse\Procedures());
        $this->add(new \Pulse\RestAuth());
    ...
    /**
     * Register Pulse's PSR-0 autoloader
     */
    public static function registerAutoloader()
    {
        spl_autoload_register(__NAMESPACE__ . "\\Pulse::autoload");
    }
    
    /**
     * Pulse ENV Procedures
     */
    ...
    /**
     * Set Pulse application name
     * @param  string $name The name of this Pulse application
     */
    ...
    *******************************************************************************/
    ...
    /**
     * Configure Pulse for a given mode
     *
     ...
     * only _after_ you initialize your Pulse app.
     *
     * @param  string $mode
    ...
    *******************************************************************************/
     ...
     * USAGE:
     *
     * Pulse::get('/foo'[, kernel, kernel, ...], callable);
     *
    ...
    protected function mapRoute($args)
    {
        ...
        $route = new \Pulse\Route($pattern, $callable, $this->settings['routes.case_sensitive']);
    ...
    public function get()
    {
        ...
        return $this->mapRoute($args)->via(\Pulse\Http\Request::METHOD_GET, \Pulse\Http\Request::METHOD_HEAD);
    }
    ...
    public function post()
    {
        ...
        return $this->mapRoute($args)->via(\Pulse\Http\Request::METHOD_POST);
    }
    ...
    public function put()
    {
        ...
        return $this->mapRoute($args)->via(\Pulse\Http\Request::METHOD_PUT);
    }
    ...
    public function patch()
    {
        ...
        return $this->mapRoute($args)->via(\Pulse\Http\Request::METHOD_PATCH);
    }
    ...
    public function delete()
    {
        ...
        return $this->mapRoute($args)->via(\Pulse\Http\Request::METHOD_DELETE);
    }
    ...
    public function options()
    {
        ...
        return $this->mapRoute($args)->via(\Pulse\Http\Request::METHOD_OPTIONS);
    }
     ...
     *
     * If the $callable parameter is null, Pulse assumes you want
     ...
     * 2. When invoking the handler:
     *
     * If the $argument parameter is not callable, Pulse assumes you want
     * to invoke an already-registered handler. If the handler has been
    ...
    *******************************************************************************/
     ...
     *
     * This method declares the View to be used by the Pulse application.
     * If the argument is a string, Pulse will instantiate a new object
     * of the same class. If the argument is an instance of View or a subclass
     * of View, Pulse will use the argument as the View.
     *
     ...
     *
     * @param  string|\Pulse\View $viewClass The name or instance of a \Pulse\View subclass
     * @return \Pulse\View
     */
    public function view($viewClass = null)
    {
        if (!is_null($viewClass)) {
            ...
            if ($viewClass instanceOf \Pulse\View) {
                $this->view = $viewClass;
            } else {
    ...
    *******************************************************************************/
    ...
    public function lastModified($time)
    {
        if (is_integer($time)) {
            ...
            throw new \InvalidArgumentException('Pulse::lastModified only accepts an integer UNIX timestamp value.');
        }
    }
    ...
    public function etag($value, $type = 'strong')
    {
        ...
        if (!in_array($type, array('strong', 'weak'))) {
            throw new \InvalidArgumentException('Invalid Pulse::etag type. Expected "strong" or "weak".');
        }
    ...
    *******************************************************************************/
    ...
    public function getCookie($name, $deleteIfInvalid = true)
    {
        ...
        if ($this->config('cookies.encrypt')) {
            $value = \Pulse\Http\Util::decodeSecureCookie(
                $value,
     ...
     * default Cookie setting values (set during Pulse::init) will be used instead.
     *
     * @param string    $name       The cookie name
     * @param string    $path       The path on the server in which the cookie will be available on
     * @param string    $domain     The domain that the cookie is available to
    ...
    *******************************************************************************/
    ...
    /**
     * Get the absolute path to this Pulse application's root directory
     *
     * This method returns the absolute path to the Pulse application's
     * directory. If the Pulse application is installed in a public-accessible
     ...
     *
     * @throws \Pulse\Exception\Stop
     */
    public function stop()
    {
        throw new \Pulse\Exception\Stop();
    }
     ...
     *
     * @throws \Pulse\Exception\Pass
     */
    public function pass()
    {
        $this->cleanBuffer();
        throw new \Pulse\Exception\Pass();
    }
    ...
    *******************************************************************************/
    ...
    {
        if (isset($this->environment['pulse.flash'])) {
            $this->environment['pulse.flash']->set($key, $value);
        }
    }
    ...
    {
        if (isset($this->environment['pulse.flash'])) {
            $this->environment['pulse.flash']->now($key, $value);
        }
    }
    ...
    {
        if (isset($this->environment['pulse.flash'])) {
            $this->environment['pulse.flash']->keep();
        }
    }
    ...
    *******************************************************************************/
     ...
     * @param \Pulse\Kernel
     */
    public function add(\Pulse\Kernel $newKernel)
    {
    ...
    *******************************************************************************/
     ...
     * Run
     *
     * This method invokes the kernel stack, including the core Pulse application;
     * the result is an array of HTTP status, header, and body. These three items
     * are returned to the HTTP client.
     */
    public function processor()
    {
        set_error_handler(array('\Pulse\Pulse', 'handleErrors'));

        //Apply final outer kernel layers
        if ($this->config('debug')) {
            //Apply pretty exceptions only in debug to avoid accidental information leakage in production
            $this->add(new \Pulse\Kernel\ErrorReporting());
        }
        ...
        \Pulse\Http\Util::serializeCookies($headers, $this->response->cookies, $this->settings);

        //Send headers
        if (headers_sent() === false) {
            //Send status
            if (strpos(PHP_SAPI, 'cgi') === 0) {
                header(sprintf('Status: %s', \Pulse\Http\Response::getMessageForCode($status)));
            } else {
                header(sprintf('HTTP/%s %s', $this->config('http.version'), \Pulse\Http\Response::getMessageForCode($status)));
            }
    ...
    public function call()
    {
        try {
            if (isset($this->environment['pulse.flash'])) {
                $this->view()->setData('flash', $this->environment['pulse.flash']);
            }
            $this->applyHook('pulse.before');
            ob_start();
            $this->applyHook('pulse.before.router');
            $dispatched = false;
            $matchedRoutes = $this->router->getMatchedRoutes($this->request->getMethod(), $this->request->getResourceUri());
            foreach ($matchedRoutes as $route) {
                try {
                    $this->applyHook('pulse.before.dispatch');
                    $dispatched = $route->dispatch();
                    $this->applyHook('pulse.after.dispatch');
                    if ($dispatched) {
                        break;
                    }
                } catch (\Pulse\Exception\Pass $e) {
                    continue;
                }
            }
            if (!$dispatched) {
            ...
            $this->applyHook('pulse.after.router');
            $this->stop();
        } catch (\Pulse\Exception\Stop $e) {
    ...
    *******************************************************************************/
     ...
     * these \ErrorException objects are then thrown and caught by Pulse's
    ...
    {
        return sprintf("<!doctype html><html lang=\"es\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><link rel=\"stylesheet\" href=\"".iasset('bootstrap.min.css')."\"><link rel=\"stylesheet\" href=\"".iasset('styles.css')."\"><title>%s</title><link rel=\"icon\" type=\"image/png\" href=\"".iasset('pulseLogo.png')."\" /><link href=\"".iasset('font-awesome.min.css')."\" rel=\"stylesheet\" type=\"text/css\"></head><body><nav class=\"navbar navbar-inverse navbar-fixed-top\"> <div class=\"container\"><div class=\"navbar-header\"><button type=\"button\" class=\"navbar-toggle collapsed\" data-toggle=\"collapse\" data-target=\"#navbar\" aria-expanded=\"false\" aria-controls=\"navbar\"><span class=\"sr-only\">Toggle navigation</span><span class=\"icon-bar\"></span><span class=\"icon-bar\"></span><span class=\"icon-bar\"></span></button><a class=\"navbar-brand\" href=\"".basePath()."\"><i class=\"fa fa-heartbeat\"></i> PULSE Framework</a></div><div id=\"navbar\" class=\"collapse navbar-collapse\"><ul class=\"nav navbar-nav\"><li><a href=\"".basePath()."\">Home</a></li><li><a href=\"".path('adashboard')."\">Admin Dashboard</a></li></ul></div></div></nav><div class=\"container\"><div class=\"starter-template\"><h1>%s</h1>%s</div></div></script></body></html>", $title, $title, $body);
    }

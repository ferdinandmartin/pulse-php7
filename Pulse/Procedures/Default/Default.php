...
function iasset($resource){
    ...
    if(substr($resource, -3)=='css') $asset=basePath().'/Pulse/Internal/css/'.$resource;
    if(substr($resource, -3)=='.js') $asset=basePath().'/Pulse/Internal/js/'.$resource;
    if(substr($resource, -3)=='peg') $asset=basePath().'/Pulse/Internal/images/'.$resource;
    if(substr($resource, -3)=='png') $asset=basePath().'/Pulse/Internal/images/'.$resource;
    if(substr($resource, -3)=='gif') $asset=basePath().'/Pulse/Internal/images/'.$resource;
    if(substr($resource, -3)=='jpg') $asset=basePath().'/Pulse/Internal/images/'.$resource;
    if(substr($resource, -3)=='svg') $asset=basePath().'/Pulse/Internal/images/'.$resource;
    
    if(empty($asset)) $asset = basePath().'/Pulse/Internal/resources/'.$resource;
...
function fklist($table) {
    global $db;
    $fkarray = array();
    $fieldsQuery = $db->prepare("SELECT * FROM pulse_tables WHERE name=:name");

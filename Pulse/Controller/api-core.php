...
$API->get('/admin/login', 
    function() use($API) {
        $API->render('Pulse/Internal/Views:login', array(
            'title' => 'Login',
            'description' => 'Welcome to Pulse admin central.',
            'error' => 'no',
        ));
    }
)->name('alogin');
...
$API->post('/admin/login', 
    function() use($API, $db) {
        ...
        $qLogin=$db->prepare("SELECT * FROM pulse_users WHERE username=:user AND password=:password");
        ...
        if($qLogin->rowCount()==1){
            ...
            $_SESSION['pulse.id']=$resultLogin[0]['id'];
            $_SESSION['pulse.user']=$resultLogin[0]['username'];
            $_SESSION['pulse.name']=$resultLogin[0]['name'];
            $_SESSION['pulse.permissions']=$resultLogin[0]['permissions'];
...
$API->get('/admin/dashboard', 
    function() use($API, $db) {
        if(isset($_SESSION['pulse.id'])){
            pathActive('dashboard');
            
            if($_SESSION['pulse.user']!='admin'):
                $navMQuery = $db->prepare("SELECT c.*, CASE WHEN isnull(is_par.id) THEN c.linkorder ELSE is_par.linkorder END as sort_order,
                                        ...
                                        FROM `pulse_nav` c
                                        LEFT JOIN `pulse_nav` is_par ON is_par.id = c.parent
                                        ORDER BY  sort_order , catid, subcat_order");
                $navMQuery->execute();
                $menuNav = $navMQuery->fetchAll(PDO::FETCH_ASSOC);
            
                $tablesQuery = $db->prepare("SELECT * FROM pulse_tables ORDER by name ASC");
                $tablesQuery->execute();
            ...
            $API->render('Pulse/Internal/Views:dashboard', array(
                'title' => 'Dashboard',
                'description' => 'Welcome to Pulse admin central.',
                'mtables' => $resTables,
                'nav' => $menuNav,
            ...
            $API->redirect($API->urlFor('alogin'));
        }
    }
)->name('adashboard');
...
$API->get('/configure', 
    function() use($API) {
        ...
        $API->render('Pulse/Internal/Views:configuration', array(
            'title' => 'Setup, Welcome to PULSE!',
            'ok' => 'no',
            'error' => 'no',
        ));
    }
...
$API->post('/configure', 
    function() use($API) {
        ...
        if($queryDB->rowCount() == 1){
            ...
            //$databaseName='pulse.'.date('Ymd').'.01';
            $databaseName=$formData->post('name');     
        ...
        $queryDBTables = $db->prepare("CREATE TABLE `pulse_events` (
...
CREATE TABLE `pulse_users` (
  ...
  `lastInvoke` datetime NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

INSERT INTO `pulse_users` (`id`, `username`, `password`, `name`, `permissions`, `creationDate`, `activeDate`, `invokeType`, `lastInvoke`) VALUES
(1, 'admin', '".md5('admin')."', 'Administrator', 1, '".date('Y-m-d')." 00:00:00', '0000-00-00 00:00:00', 'app', '0000-00-00 00:00:00'),
(2, 'user', '".md5('user')."', 'User', 3, '".date('Y-m-d')." 00:00:00', '0000-00-00 00:00:00', 'app', '0000-00-00 00:00:00');
    
ALTER TABLE `pulse_events`
  ADD PRIMARY KEY (`id`),
  ADD KEY `table_id_fk` (`table_id_fk`);

ALTER TABLE `pulse_fields`
  ADD PRIMARY KEY (`id`),
  ADD KEY `table_id_fk` (`table_id_fk`);

ALTER TABLE `pulse_nav`
  ADD PRIMARY KEY (`id`),
  ADD KEY `table_id_fk` (`table_id_fk`);

ALTER TABLE `pulse_tables`
  ADD PRIMARY KEY (`id`);
...
ALTER TABLE `pulse_events`
  ADD CONSTRAINT `tables_events_constraint` FOREIGN KEY (`table_id_fk`) REFERENCES `pulse_tables` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `pulse_fields`
  ADD CONSTRAINT `tableid_fieldtableid` FOREIGN KEY (`table_id_fk`) REFERENCES `pulse_tables` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `pulse_nav`
  ADD CONSTRAINT `navigation_tables_const` FOREIGN KEY (`table_id_fk`) REFERENCES `pulse_tables` (`id`) ON DELETE SET NULL ON UPDATE SET NULL;
  
CREATE TRIGGER `linkOrderNumber` BEFORE INSERT ON `pulse_nav` FOR EACH ROW BEGIN
	DECLARE lastOrder INT DEFAULT 0;
    
    IF NEW.parent IS NULL THEN
      SELECT linkorder INTO lastOrder FROM pulse_nav WHERE parent IS NULL ORDER by linkorder DESC LIMIT 1;
    ELSE 
      SELECT linkorder INTO lastOrder FROM pulse_nav WHERE parent = NEW.parent ORDER by linkorder DESC LIMIT 1;
    END IF;
    
    SET NEW.linkorder = lastOrder + 1;
END ;

CREATE TRIGGER `linkOrderNumberUpdate` BEFORE UPDATE ON `pulse_nav` FOR EACH ROW BEGIN
	DECLARE lastOrder INT DEFAULT 0;
...
$db= null;
        ...
        $fichero = 'Pulse/Config/Config.php';
        // Escribe el contenido al fichero
        file_put_contents($fichero, $contentConfig);

        // replace this example code with whatever you need
        $API->render('Pulse/Internal/Views:configuration', array(
            'title' => 'PULSE configured!',
            'ok' => 'ok',
            'error' => 'no',
        ));
        } else {
            $API->render('Pulse/Internal/Views:configuration', array(
                'title' => 'Unable to connect to Database, PULSE Framework',
                'ok' => 'no',
...
$API->get('/admin/invoke', 
    function() use($API, $db) {
            ...
            $API->render('Pulse/Internal/Views:invoke', array(
...
$API->get('/admin/invoke/{type}/start', 
    function($type) use($API, $db) {
            ...
            $qUser=$db->prepare("SELECT * FROM pulse_users WHERE id=:id");
            $qUser->execute(array(
                    ':id'=> $_SESSION['pulse.id']
                    ));

            $resultUser = $qUser->fetchAll(PDO::FETCH_ASSOC);
        
            $API->render('Pulse/Internal/Views:invokeStart', array(
                'title' => 'API Invocation',
...
$API->post('/admin/invoke/{type}/start', 
    function($type) use($API, $db) {
            ...
            require_once 'Pulse/Invocations/'.$invocationFile.'.php';
...
$API->get('/admin/invoke/{type}/end', 
    function($type) use($API, $db) {
            ...
            $API->render('Pulse/Internal/Views:invokeResult', array(
                'title' => 'API Invocation',
                'description' => 'Invocation finished.',
                'type' => $type,
                'name' => $name,
...
$API->get('/admin/documentation', 
    function() use($API) {
            ...
            $API->render('Pulse/Internal/Views:documentation', array(
                'title' => 'PULSE Documentation',
                'description' => 'Learn to use Pulse, Although you don\'t really need it..',
            ));
    }
)->name('documentation-pulse');
...
$API->get('/admin/central/{id}/{permalink}', 
    function($id, $permalink) use($API, $db) {
            ...
            $tablesMQuery = $db->prepare("SELECT * FROM pulse_tables ORDER by name ASC");
            $tablesMQuery->execute();
            $menuTables = $tablesMQuery->fetchAll(PDO::FETCH_ASSOC);
            
            $tablesQuery = $db->prepare("SELECT * FROM pulse_tables WHERE id=:id ORDER by name ASC");
            $tablesQuery->execute(array('id'=>$id));
            $resTables = $tablesQuery->fetchAll(PDO::FETCH_ASSOC);
            
            $fieldsQuery = $db->prepare("SELECT * FROM pulse_fields WHERE table_id_fk=:id ORDER by win_order ASC");
            ...
            $API->render('Pulse/Internal/Views:centralList', array(
...
$API->get('/admin/central/add/{id}/{permalink}', 
    function($id, $permalink) use($API, $db) {
            ...
            $API->render('Pulse/Internal/Views:centralForm', array(
...
$API->get('/admin/central/edit/{id}/{id_table}/{permalink}', 
    function($id, $idTable, $permalink) use($API, $db) {
            ...
            $API->render('Pulse/Internal/Views:centralEditForm', array(

    ...
    function() use($API, $db) {
        if(isset($_SESSION['pulse.id'])){
            ...
            $tablesQuery = $db->prepare("SELECT p.id, p.name, p.description, p.win_name, p.creationDate,
                                        (SELECT COUNT(id) FROM pulse_fields WHERE table_id_fk = p.id) FieldsIn FROM pulse_tables p ORDER by p.name ASC");
            $tablesQuery->execute();
            $resTables = $tablesQuery->fetchAll(PDO::FETCH_ASSOC);
            
            $API->render('Pulse/Internal/Views:tables', array(
                'title' => 'Tables',
...
$API->get('/admin/tables/add', 
    function() use($API) {
            pathActive('tables');
            $API->render('Pulse/Internal/Views:addTables', array(
...
$API->post('/admin/tables/add', 
    function() use($API, $db) {
        ...
        $queryInsert=$db->prepare("INSERT INTO pulse_tables(name,description,win_name,win_description,table_name_field,creationDate)
...
$API->get('/admin/tables/edit/{table_id}', 
    function($tableID) use($API, $db) {
        pathActive('tables');
        $query1 = $db->prepare("SELECT * FROM pulse_tables WHERE id=:tabid ORDER by id ASC");
        $query1->execute(array(
            'tabid' => $tableID
        ));
        $result1 = $query1->fetchAll(PDO::FETCH_ASSOC);
        
        $API->render('Pulse/Internal/Views:editTables', array(
...
$API->post('/admin/tables/edit/{table_id}', 
    function($tableID) use($API, $db) {
        ...
        $queryInsert=$db->prepare("UPDATE pulse_tables SET name=:name,description=:description,win_name=:win_name,win_description=:win_description,table_name_field=:table_name_field WHERE id=:id");
...
$API->get('/admin/tables/delete/{table_id}', 
    function($tableID) use($API, $db) {
        ...
        $query2 = $db->prepare("DELETE FROM pulse_tables WHERE id=:tabid");
...
$API->get('/admin/tables/relations/add/{table_id}', 
    function($tableID) use($API, $db) {
        ...
        $query2 = $db->prepare("SELECT * FROM pulse_tables WHERE id!=:tabid ORDER by id ASC");
        $query2->execute(array(
            'tabid' => $tableID
        ));
        $result2 = $query2->fetchAll(PDO::FETCH_ASSOC);
        
        $API->render('Pulse/Internal/Views:addRelations', array(
...
$API->post('/admin/tables/relations/add/{table_id}', 
    function($tableID) use($API, $db) {
        pathActive('tables');
        $formData=$API->request;
        $query1 = $db->prepare("SELECT * FROM pulse_tables WHERE name=:tabname ORDER by id ASC");
        $query1->execute(array(
            'tabname' => $formData->post('destTable')
        ));
        $result1 = $query1->fetchAll(PDO::FETCH_ASSOC);
        
        $queryInsert=$db->prepare("INSERT INTO pulse_fields(table_id_fk,name,comment,win_name,win_description,win_order,win_type,win_code,creationDate,author)
        ...
        $queryInsert->execute(
                array(
                    ...
                    ':creationDate'=> date('Y-m-d H:i:s'),
                    ':author'=> $_SESSION['pulse.user'],
                    )
                );
        
        $queryInsertLocal=$db->prepare("INSERT INTO pulse_fields(table_id_fk,name,comment,win_name,win_description,win_order,win_type,win_code,creationDate,author)
                                values (:fkid,:name,:comment,:win_name,:win_description,:win_order,:win_type,:win_code,:creationDate,:author);");
...
$API->get('/admin/tables/fields/{table_id}', 
    function($tableID) use($API, $db) {
        ...
        $query2 = $db->prepare("SELECT * FROM pulse_fields WHERE table_id_fk=:tabid ORDER by win_order ASC");
        $query2->execute(array(
            'tabid' => $tableID
        ));
        $result2 = $query2->fetchAll(PDO::FETCH_ASSOC);
        
        $query3 = $db->prepare("SELECT * FROM pulse_events WHERE table_id_fk=:tabid ORDER by time,type ASC");
        $query3->execute(array(
            'tabid' => $tableID
        ));
        $result3 = $query3->fetchAll(PDO::FETCH_ASSOC);
        
        $API->render('Pulse/Internal/Views:fields', array(
...
$API->get('/admin/tables/fields/add/{table_id}', 
    function($tableID) use($API) {
            pathActive('tables');
            $API->render('Pulse/Internal/Views:addFields', array(
...
$API->get('/admin/tables/{table_id}/fields/edit/{field_id}', 
    function($tableID, $fieldID) use($API, $db) {
        ...
        $query2 = $db->prepare("SELECT * FROM pulse_fields WHERE id=:fieid ORDER by id ASC");
        $query2->execute(array(
            'fieid' => $fieldID
        ));
        $result2 = $query2->fetchAll(PDO::FETCH_ASSOC);
        
        $API->render('Pulse/Internal/Views:editFields', array(
...
$API->post('/admin/tables/{table_id}/fields/edit/{field_id}', 
    function($tableID, $fieldID) use($API, $db) {
        pathActive('tables');
        $formData=$API->request;
        $queryInsert=$db->prepare("UPDATE pulse_fields SET name=:name,comment=:comment,win_name=:win_name,win_description=:win_description,win_type=:win_type,win_order=:win_order,win_code=:win_code WHERE id=:id");
...
$API->get('/admin/tables/{table_id}/fields/delete/{field_id}', 
    function($tableID, $fieldID) use($API, $db) {
        ...
        $query3 = $db->prepare("DELETE FROM pulse_fields WHERE id=:fieid");
        ...
        if($status){
            ...
            if($endFieldName=='fk'){
                $query4 = $db->prepare("DELETE FROM pulse_fields WHERE win_description=:windesc");
...
$API->get('/admin/tables/trigger/add/{table_id}', 
    function($tableID) use($API) {
            pathActive('tables');
            $API->render('Pulse/Internal/Views:addTrigger', array(
...
DELIMITER ;");
        ...
        if($status2){
            $queryInsert = $db->prepare("INSERT INTO pulse_events(table_id_fk,name,time,type,code,creationDate)
...
$API->get('/admin/tables/{table_id}/trigger/edit/{event_id}', 
    function($tableID, $eventID) use($API, $db) {
        ...
        $query2 = $db->prepare("SELECT * FROM pulse_events WHERE id=:eveid ORDER by id ASC");
        $query2->execute(array(
            'eveid' => $eventID
        ));
        $result2 = $query2->fetchAll(PDO::FETCH_ASSOC);
        
        $API->render('Pulse/Internal/Views:editTrigger', array(
...
$API->post('/admin/tables/{table_id}/trigger/edit/{event_id}', 
    function($tableID, $eventID) use($API, $db) {
        ...
        $queryInsert=$db->prepare("UPDATE pulse_events SET code=:code WHERE id=:id");
...
$API->get('/admin/tables/{table_id}/trigger/delete/{event_id}', 
    function($tableID, $eventID) use($API, $db) {
        ...
        $query3 = $db->prepare("DELETE FROM pulse_events WHERE id=:eveid");
        $status = $query3->execute(array(
                        'eveid' => $eventID
                    ));

...
$API->get('/admin/nav', 
    function() use($API, $db) {
        if(isset($_SESSION['pulse.id'])){
            ...
            $API->render('Pulse/Internal/Views:nav', array(
...
$API->get('/admin/nav/add', 
    function() use($API, $db) {
            ...
            $query2 = $db->prepare("SELECT * FROM pulse_tables ORDER by id ASC");
            $query2->execute();
            $result2 = $query2->fetchAll(PDO::FETCH_ASSOC);
            
            $query3 = $db->prepare("SELECT * FROM pulse_nav ORDER by id ASC");
            $query3->execute();
            $result3 = $query3->fetchAll(PDO::FETCH_ASSOC);
        
            $API->render('Pulse/Internal/Views:addNav', array(
...
$API->post('/admin/nav/add', 
    function() use($API, $db) {
        ...
        $queryInsert=$db->prepare("INSERT INTO pulse_nav(name,description,parent,table_id_fk,creationDate,icon)
...
$API->get('/admin/nav/edit/{nav_id}', 
    function($navID) use($API, $db) {
        pathActive('nav');
        $query1 = $db->prepare("SELECT * FROM pulse_nav WHERE id=:navid ORDER by id ASC");
        ...
        $query3 = $db->prepare("SELECT * FROM pulse_nav WHERE parent IS NULL ORDER by id ASC");
        $query3->execute();
        $result3 = $query3->fetchAll(PDO::FETCH_ASSOC);
        
        $API->render('Pulse/Internal/Views:editNav', array(
...
$API->post('/admin/nav/edit/{nav_id}', 
    function($navID) use($API, $db) {
        ...
        $queryInsert=$db->prepare("UPDATE pulse_nav SET name=:name,description=:description,parent=:parent,table_id_fk=:table_id_fk,icon=:icon WHERE id=:id");
        ...
        if($result1[0]['parent']!=$formData->post('navParent')){
            if($result1[0]['parent']==null){
                $queryParent = $db->prepare("SELECT * FROM pulse_nav WHERE parent IS NULL  ORDER by linkorder ASC");
                $queryParent->execute(array(
                    'oldParent' => $result1[0]['parent']
                ));
            } else {
                $queryParent = $db->prepare("SELECT * FROM pulse_nav WHERE parent=:oldParent ORDER by linkorder ASC");
                $queryParent->execute(array(
                    'oldParent' => $result1[0]['parent']
                ));
            }
            ...
            foreach ($resOldParent as $key => $value) {
                $reOrderParent = $db->prepare("UPDATE pulse_nav SET linkorder=:newOrder WHERE id=:navID;");
...
$API->get('/admin/nav/delete/{nav_id}', 
    function($navID) use($API, $db) {
        ...
        $query2 = $db->prepare("DELETE FROM pulse_nav WHERE id=:navid");
...
$API->get('/admin/nav/up/{nav_id}', 
    function($navID) use($API, $db) {
        ...
        if($result1[0]['parent']==null){
            $query2 = $db->prepare("SELECT * FROM pulse_nav WHERE linkorder=:lorder AND parent IS NULL LIMIT 1");
            $query2->execute(array(
                'lorder' => ($result1[0]['linkorder']-1),
            ));
        } else {
            $query2 = $db->prepare("SELECT * FROM pulse_nav WHERE linkorder=:lorder AND parent=:lparent LIMIT 1");
            $query2->execute(array(
                'lorder' => ($result1[0]['linkorder']-1),
        ...
        $query3 = $db->prepare("UPDATE pulse_nav SET linkorder=linkorder-1 WHERE id=:navid; "
                             . "UPDATE pulse_nav SET linkorder=linkorder+1 WHERE id=:prenavid;");
...
$API->get('/admin/config', 
    function() use($API, $db) {
            ...
            $query = $db->prepare("SELECT * FROM pulse_users WHERE id=".$_SESSION['pulse.id']);
            $query->execute();
            $result = $query->fetchAll(PDO::FETCH_ASSOC);
        
            $API->render('Pulse/Internal/Views:config', array(
                'title' => 'Configuration',
                'description' => 'Change Pulse configuration details.',
                'conf' => $result,
            ));
    }
)->name('config');
...
$API->post('/admin/config', 
    function() use($API, $db) {
        ...
        $queryInsert=$db->prepare("UPDATE pulse_users SET name=:name,permissions=:permissions,invokeType=:invokeType WHERE id=:id");
        $status=$queryInsert->execute(
                array(
        ...
        if($formData->post('pass')!=''){
            if($formData->post('pass')==$formData->post('cpass')){
                $queryUpdatePass=$db->prepare("UPDATE pulse_users SET password=:pass WHERE id=:id");

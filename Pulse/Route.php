...
class Route
{
    ...
    /**
     * @var array[Callable] Kernel to be run before only this route instance
     */
     ...
     * Set kernel
     *
     * This method allows kernel to be assigned to a specific Route.
     * If the method argument `is_callable` (including callable arrays!),
     * we directly append the argument to `$this->kernel`. Else, we
     * assume the argument is an array of callables and merge the array
     * with `$this->kernel`.  Each kernel is checked for is_callable()
    ...
    public function setKernel($kernel)
    {
        if (is_callable($kernel)) {
            $this->kernel[] = $kernel;
        } elseif (is_array($kernel)) {
            foreach ($kernel as $callable) {
                if (!is_callable($callable)) {
                    throw new \InvalidArgumentException('All Route kernel must be callable');
                }
            }
            $this->kernel = array_merge($this->kernel, $kernel);
        } else {
            throw new \InvalidArgumentException('Route kernel must be callable or an array of callables');
        }
    ...
    /**
     * Matches URI, like TWIG with {var}
     *
    ...
    {
        $this->setName($name);
        setPathName($name,basePath().$this->getPattern());
     ...
     * Dispatch route
     *
     * This method invokes the route object's callable. If kernel is
     * registered for the route, each callable kernel is invoked in
     * the order specified.
     *
     * @return bool
     */

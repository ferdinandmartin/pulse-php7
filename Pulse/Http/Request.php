...
class Request
{
     ...
     * @var \Pulse\Environment
     */
    protected $env;

    /**
     * HTTP Headers
     * @var \Pulse\Http\Headers
     */
    public $headers;
    ...
    /**
     * Constructor
     * @param \Pulse\Environment $env
     */
    public function __construct(\Pulse\Environment $env)
    {
        $this->env = $env;
        $this->headers = new \Pulse\Http\Headers(\Pulse\Http\Headers::extract($env));
        $this->cookies = new \Pulse\Security\Set(\Pulse\Http\Util::parseCookieHeader($env['HTTP_COOKIE']));
    }
    ...
    /**
     * Is this an XHR request? (alias of Pulse_Http_Request::isAjax)
     * @return bool
     */
    ...
    {
        if (!isset($this->env['pulse.request.query_hash'])) {
            $output = array();
            if (function_exists('mb_parse_str') && !isset($this->env['pulse.tests.ignore_multibyte'])) {
                mb_parse_str($this->env['QUERY_STRING'], $output);
            } else {
                parse_str($this->env['QUERY_STRING'], $output);
            }
            $this->env['pulse.request.query_hash'] = Util::stripSlashesIfMagicQuotes($output);
        }
        if ($key) {
            if (isset($this->env['pulse.request.query_hash'][$key])) {
                return $this->env['pulse.request.query_hash'][$key];
            } else {
                return $default;
            }
        } else {
            return $this->env['pulse.request.query_hash'];
        }
    }
    ...
    {
        if (!isset($this->env['pulse.input'])) {
            throw new \RuntimeException('Missing pulse.input in environment variables');
        }
        if (!isset($this->env['pulse.request.form_hash'])) {
            $this->env['pulse.request.form_hash'] = array();
            if ($this->isFormData() && is_string($this->env['pulse.input'])) {
                $output = array();
                if (function_exists('mb_parse_str') && !isset($this->env['pulse.tests.ignore_multibyte'])) {
                    mb_parse_str($this->env['pulse.input'], $output);
                } else {
                    parse_str($this->env['pulse.input'], $output);
                }
                $this->env['pulse.request.form_hash'] = Util::stripSlashesIfMagicQuotes($output);
            } else {
                $this->env['pulse.request.form_hash'] = Util::stripSlashesIfMagicQuotes($_POST);
            }
        }
        if ($key) {
            if (isset($this->env['pulse.request.form_hash'][$key])) {
                return $this->env['pulse.request.form_hash'][$key];
            } else {
                return $default;
            }
        } else {
            return $this->env['pulse.request.form_hash'];
        }
    }

    /**
     * Fetch PUT data (alias for \Pulse\Http\Request::post)
     * @param  string           $key
     * @param  mixed            $default Default return value when key does not exist
     * @return array|mixed|null
     */
    ...
    /**
     * Fetch PATCH data (alias for \Pulse\Http\Request::post)
     * @param  string           $key
     * @param  mixed            $default Default return value when key does not exist
     * @return array|mixed|null
     */
    ...
    /**
     * Fetch DELETE data (alias for \Pulse\Http\Request::post)
     * @param  string           $key
     * @param  mixed            $default Default return value when key does not exist
     * @return array|mixed|null
     */
    ...
    {
        $method = isset($this->env['pulse.method_override.original_method']) ? $this->env['pulse.method_override.original_method'] : $this->getMethod();
    ...
    {
        return $this->env['pulse.url_scheme'];
    }
    ...
    /**
     * LEGACY: Get Root URI (alias for Pulse_Http_Request::getScriptName)
     * @return string
     */
    ...
    /**
     * LEGACY: Get Resource URI (alias for Pulse_Http_Request::getPathInfo)
     * @return string
     */

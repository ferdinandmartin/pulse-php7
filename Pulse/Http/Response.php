...
class Response implements \ArrayAccess, \Countable, \IteratorAggregate
{
     ...
     * @param \Pulse\Http\Headers|array $headers The HTTP response headers
     */
    public function __construct($body = '', $status = 200, $headers = array())
    {
        $this->setStatus($status);
        $this->headers = new \Pulse\Http\Headers(array('Content-Type' => 'text/html'));
        $this->headers->replace($headers);
        $this->cookies = new \Pulse\Http\Cookies();
        $this->write($body);
    }
     ...
     * if available or directly to the Pulse run method.
     *
     ...
     * Set cookie
     *
     * Instead of using PHP's `setcookie()` function, Pulse manually constructs the HTTP `Set-Cookie`
     ...
     * relying on PHP's native implementation, Pulse allows kernel the opportunity to massage or
    ...
    /**
     * DEPRECATION WARNING! ArrayAccess interface will be removed from \Pulse\Http\Response.
    ...
    /**
     * DEPRECATION WARNING! Countable interface will be removed from \Pulse\Http\Response.
    ...
    /**
     * DEPRECATION WARNING! IteratorAggregate interface will be removed from \Pulse\Http\Response.
     * Iterate `headers` or `cookies` properties directly.
     *
     * Get Iterator
     *
     * This returns the contained `\Pulse\Http\Headers` instance which
     * is itself iterable.
     *
     * @return \Pulse\Http\Headers
     */

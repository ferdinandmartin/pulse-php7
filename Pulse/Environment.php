...
class Environment implements \ArrayAccess, \IteratorAggregate
{
     ...
     * variables by using `\Pulse\Environment::mock()` instead.
     *
    ...
    public static function mock($userSettings = array())
    {
        $defaults = array(
            ...
            'USER_AGENT' => 'Pulse Framework',
            'REMOTE_ADDR' => '127.0.0.1',
            'pulse.url_scheme' => 'http',
            'pulse.input' => '',
            'pulse.errors' => @fopen('php://stderr', 'w')
        );
    ...
    private function __construct($settings = null)
    {
        if ($settings) {
            ...
            $headers = \Pulse\Http\Headers::extract($_SERVER);
            foreach ($headers as $key => $value) {
                $env[$key] = $value;
            }

            //Is the application running under HTTPS or HTTP protocol?
            $env['pulse.url_scheme'] = empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] === 'off' ? 'http' : 'https';
            ...
            $env['pulse.input'] = $rawInput;

            //Error stream
            $env['pulse.errors'] = @fopen('php://stderr', 'w');
